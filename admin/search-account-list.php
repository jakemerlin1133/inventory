...
<body class="bg-gradient-to-r from-sky-300 via-blue-200 to-sky-300">
    ...
    <table class="w-[95%] mb-8 mt-2 mx-auto font-semibold text-center text-md text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            <?php
                if(!empty($search_account_row)){
                    if(mysqli_num_rows($search_account_query) > 0) {
                
            do {
                  
                $activation = $search_account_row['activation'];
            ?>
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    <td class="px-6 py-4">
                        <?php echo $search_account_row['username'] ?>
                    </td>

                    <td class="px-6 py-4">
                        <?php echo $search_account_row['password'] ?>
                    </td>

                    <td class="px-6 py-4">
                        <?php echo $search_account_row['user_status'] ?>
                    </td>

                    <td class="px-6 py-4">
                        <?php echo $search_account_row['first_name'] ?>
                    </td>

                    <td class="px-6 py-4">
                        <?php echo $search_account_row['lastname'] ?>
                    </td>

                    <td class="px-6 py-4">
                        <?php echo date("F-d-Y: h:i:s A", strtotime($search_account_row['date_created'])) ?>
                    </td>

                    <?php
                        if($activation == "activate"){  
                    ?>
                    <td class="px-6 py-4 text-green-600 font-bold px-2"><?php echo $search_account_row['activation'] ?></td>
                    <?php
                        }else{
                    ?>
                        <td class="px-6 py-4 text-red-600 font-bold"><?php echo $search_account_row['activation'] ?></td>
                        <?php
                        }
                        ?>
                    ...
                    <td class="px-6 py-4 ">
                        <div>
                            <form method="POST" action="activation.php?ID=<?php echo $search_account_row['user_ID'] ?>">
                                <input type="hidden" name="firstname" value="<?php echo $search_account_row['first_name'] ?>">
                                <input type="hidden" name="lastname" value="<?php echo $search_account_row['lastname'] ?>">
                                <input type="hidden" name="username" value="<?php echo $search_account_row['username'] ?>">
                                <input type="hidden" name="activation" value="<?php echo $search_account_row['activation'] ?>">
                                <?php
                                if ($activation == 'activate') {
                                ?>
            ...
            <?php
             ...
             while ($search_account_row = mysqli_fetch_assoc($search_account_query));
        }
    }
