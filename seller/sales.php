        ...
        <div id="shopping-cart" class="fixed right-[25px] top-[15px]">
            <a href="cart.php"><i class="relative fa-solid fa-cart-shopping rounded-full bg-sky-600 text-white p-4 text-3xl"></i>
                <?php if (isset($cart_quantity)) { ?>
                    <div class="absolute w-8 text-center rounded-full top-[-10px] right-[-6px] bg-red-700 px-1 py-1 text-[15px] text-white"><?php echo $cart_quantity, $plus ?></div>
                <?php } ?>
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-5 mx-4">
        ...
        do {
            if (isset($item_row)) {
                ...
                $total_stocks_query = "SELECT SUM(Stocks) AS total_stocks FROM inventory WHERE Item_ID = $item_id";
                $total_stocks_result = mysqli_query($con, $total_stocks_query);
                $total_stocks_row = mysqli_fetch_assoc($total_stocks_result);

                if($total_stocks_row['total_stocks'] > 0){
        ?>
                <div class="mt-4">
                    <div class="bg-sky-600 p-2 rounded-xl m-2 z-[-3] m-4 visible">
                        <input type="hidden" name="Item_ID" value="<?php echo $item_id ?>">
                        <div class="h-full w-full mx-auto">
                            <img class="rounded-[35px] mx-auto p-6 h-[320px] w-[320px]" src="../img/<?php echo $item_row['Item_Image'] ?>">
                        </div>
                        <div class="mx-auto text-center">
                            <div class="h-full w-full">
                                <h1 class="text-center text-zinc-100 text-3xl mt-2 w-full" style="font-family: 'Secular One', sans-serif;"><?php echo $item_row['Item_Name'] ?></h1>
                                <input type="hidden" name="Item_Name" value="<?php echo $item_row['Item_Name'] ?>">
                            </div>
                            <div class="h-full w-full">
                        ...
                        <div class="flex text-right mt-2 w-full">
                            <div class="text-zinc-100 text-4xl py-2 mx-auto font-extralight" style="font-family: 'Secular One', sans-serif;"> ₱ <?php echo $item_row['Item_Price'] ?>
                                <input type="hidden" name="Item_Price" value="<?php echo $item_row['Item_Price'] ?>">
                            </div>
                            <div class="my-auto">
                                <button data-modal-target="cart-<?php echo $item_id ?>" data-modal-toggle="cart-<?php echo $item_id ?>" class="block font-semibold text-white bg-cyan-950 hover:bg-cyan-950 focus:outline-none rounded-lg text-md px-6 py-2.5 text-center mr-2 hover:opacity-75" type="submit">Add to Cart</button>
                            </div>
                        </div>
                    </div>
                    <!-- Main modal -->
                    <form method="POST" action="cart.php?ID=<?php echo $item_row['Item_ID'] ?>">
                        <div id="cart-<?php echo $item_id ?>" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative w-full max-w-md max-h-full">
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-hide="cart-<?php echo $item_id ?>">
                                    ...
                                    <input type="hidden" name="Item_Name" value="<?php echo $item_row['Item_Name'] ?>">
                                    <input type="hidden" name="Item_Price" value="<?php echo $item_row['Item_Price'] ?>">
                                    <input type="hidden" name="Item_Image" value="<?php echo $item_row['Item_Image'] ?>">
                                    ...
                                    <div class="px-6 py-6 lg:px-8">
                                        ...
                                        <div>
                                            <label for="size-<?php echo $item_id ?>" class="block my-2 text-sm font-medium text-gray-900 dark:text-white">Size</label>
                                            <select name="size" id="size-<?php echo $item_id ?>" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:border-gray-300 focus:outline-gray-300 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white">
